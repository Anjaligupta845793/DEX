...
contract Dex {
        ...
        poolMustExist(from, to)
    {
        ...
        ERC20 contractA = ERC20(from);
        ERC20 contractB = ERC20(to);
        require( contractA.transferFrom(msg.sender, address(this), amount),  "transfer was not successfull");
        require(contractB.transfer(msg.sender, outputTokens),"transfer was not successfull");
    }
    ...
    function lpbalances(address lp , address tokenA , address tokenB) external view returns(uint){
          Pool storage pool = _getPool(tokenA, tokenB);
         return ( pool.lpBalances[lp]);

    }
    function getTotalLpTokens(address tokenA , address tokenB) external view returns(uint){
        Pool storage pool = _getPool(tokenA, tokenB);
        return(pool.totalLpTokens) ;   
    }

    function getbalance(address tokenA , address tokenB) external view returns(uint tokenAamount,uint tokenBamount){
            Pool storage pool = _getPool(tokenA, tokenB);
